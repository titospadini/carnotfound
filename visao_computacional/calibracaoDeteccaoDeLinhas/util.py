...
def mantemFps(tempoAtual,ultimoTempo,fps=30):
    """mantém uma fps mínima, por padrão mantém 30 fps"""
    tempoPorFrame = 1.0/fps
    deltaTempo = tempoAtual - ultimoTempo
    tempoAjuste = tempoPorFrame - deltaTempo
    if tempoAjuste > 0:
        sleep(tempoAjuste)
        
def nothing(x):
    """função que faz nada para passar para os sliders com ação automática"""
    pass
...
def desenhaRetangulos(imgOriginal,thresh,contourAreaMin=0,contourAreaMax=20000,anguloMin=0,anguloMax=180):
    """Recebe a imagem original e uma imagem binarizada. Na imagem binarizada são encontrados retângulos, que são desenhados na imagem original, caso se encontrem nos limites dados pelos argumentos
    """
    ...
    for contour in contours:
        ...
        if areaContour<contourAreaMin or areaContour>contourAreaMax:
            continue #vai para próximo item do loop for sem desenhar o retangulo
        
        retangulo = cv.minAreaRect(contour)
        coordY = (int)(retangulo[0][0])
        coordX = (int)(retangulo[0][1])
        angulo = anguloDoRetangulo(retangulo)
        
        if angulo<anguloMin or angulo>anguloMax:
            continue #vai para próximo item do loop for sem desenhar o retangulo
        escreveTexto(img,(str)(angulo),(coordY,coordX))
...
def calculaMatrizBEV(dstCols,dstRows,margemCols,margemRows):
    """calcula a matriz para realizar a transformação do ponto de vista para uma perspectiva birdEyeView (no momento calibrada para um vídeo específico)
    """
    rowHigh = dstRows
...
#funções abaixo só são realizadas se o tamanho do kernel não for -1, apenas para calibração
